...
ワイプオフでは、このコンパクトなバッファを `sdr_cpx16_t` 作業サンプルへ変換する。
標準相関器ではこの変換は相関と融合されており、キャッシュに収まるチャンク単位で
ワイプオフと相関を連続して行うため、全長のベースバンドバッファは生成しない。
L6 CSK パス (`L6D`/`L6E`) と複素コードパス (`E5ABQ`) では、従来どおり
1 コード周期分の `sdr_cpx16_t` 作業バッファへ変換する。相関器出力は `sdr_cpx_t`
に積算する。これは FFT とループ弁別器で使う浮動小数点複素型である。
...
QZSS `L6D` と `L6E` は通常信号と同様に信号捕捉されるが、CSK シンボル構造のため
ロック後にもコードシフト探索が必要になる。信号追尾では、この目的専用の小さな
チップ領域 FFT 相関を実行する (4.16 節参照)。
...
リサンプリングし、1 バイトの `int8_t` (値 -1/0/1) として保存する。これにより
複素表現に比べてバンクのメモリ量が半減する。各信号追尾エポックでは、現在の
小数コードオフセットに対応するバンクを選択する。
...
ほとんどの信号では、信号追尾に搬送波ワイプオフ融合型の時間領域標準相関器を使う。
IF サンプルをチャンク単位で搬送波ワイプオフし、そのまま prompt、early、late、noise、
およびオプションの BOC 監視レプリカと相関を取る 1 パス構成のため、作業サンプルが
キャッシュに留まり、全長のベースバンドバッファを相関器ごとに読み直すことがない。
相関器はコード周期をコード折り返し点で 2 つに分け、
...
`L6D` と `L6E` では、CSK シンボルを候補コードシフトにわたるチップ領域 FFT 相関で
検出し、相関器出力は CSK シフト済みコードに対する標準相関器で生成する
(4.16 節参照)。
...
| `csk_ref` | フレーム同期後の L6 CSK コードシフト基準 (-1: 未設定) |
...
搬送波ワイプオフは `sdr_mix_carr()` が行うか、`sdr_corr_std()` の内部に融合
されている。どちらも `sdr_buff_t` 内のコンパクトな IF サンプル、開始インデックス、
サンプル数、サンプルレート、搬送波周波数を受け取り、`sdr_cpx16_t` サンプルを
生成する。`sdr_mix_carr()` は FFT 相関器と複素コード相関器用に作業バッファへ
書き出し、`sdr_corr_std()` は全長バッファを生成せずチャンク単位で直接消費する。
...
通常の信号では、`sdr_corr_std()` が搬送波ワイプオフと要求された位置の相関を
融合した 1 パスで実行する。キャッシュに収まるチャンク単位で IF サンプルを
ミックスし、そのまま全相関器位置へ積算する。各位置について、
...
QZSS L6 信号は CSK (code shift keying) を使う。10230 チップのコードが 4 ms シンボル毎に
8 ビットのシンボル値だけ巡回シフトされる。送信波形では L6D と L6E がチップレートの
2 倍で時分割多重されており、各コードチップは 2 つのサブチップスロットから成る。
L6D は偶数スロット、L6E は奇数スロットを占める。

シンボル検出とループ追尾は分離されている。

1. 1 コード周期を搬送波ワイプオフし、受信機チャネルの作業バッファへ書き込む。
2. `sdr_bin_csk()` が信号側 TDM スロットのサンプルをチップ毎に累積する。
   ビン境界に端数コードオフセットを反映することで、サブサンプル精度のコード整列を
   行う。出力は 10230 チップの複素系列である。
3. この系列を、両側に `CSK_WIN = 280` チップずつ周期延長したコードレプリカと
   固定サイズ FFT (`CSK_NFFT = 10800`) で巡回相関する。この構成では ±280 チップの
   探索窓内の全シフトについて、折り返し混入のない正確な巡回相関値が得られる。
   候補シフトにわたるピークが CSK シフトを与え、シンボルを航法データシンボル
   バッファへ追加する。
4. E/P/L/N 相関器出力は、検出シフトをコードオフセットに加算した標準時間領域
   相関器で生成する。これにより FLL、PLL、DLL、C/N0 は CSK シフト済みコードの
   真の (補間ではない) 相関値で動作する。標準相関器はコード極性固定 (`pol = 1`)
   で呼び出す。L6 の相関窓はシンボル整列かつ円状であり、コード巻き戻し点での
   データビット反転は起こり得ない。また通常の反転検出は、片側の部分相関が雑音
   支配になるとランダムに誤動作するためである。

相関 FFT サイズが固定のため、シンボル検出コストはサンプリング周波数に依存しない。
サンプリング周波数に比例するのは搬送波ワイプオフ、チップビニング、標準相関器のみ
である。

フレーム同期前は、信号捕捉時に確立したコード位相基準が捕捉時に送信されていた
未知シンボル分だけオフセットしているため、ピーク探索は ±255 チップのシフトを
カバーする。この窓は各シンボル値を 2 回含む。シフト `s` と `s - 256` は同一シンボル
に復号されるが、ループ相関器を真のピークに置けるのは一方だけである。L6 航法データ
デコーダがフレーム同期を達成して定数シンボルオフセット (`nav->coff` に格納) を
復元すると、受信機チャネルは `trk->csk_ref` をアンカーし、探索を 256 シフト
`[csk_ref - 255, csk_ref]` に狭めてエイリアス候補を排除する。アンカーはフレーム
同期喪失時にクリアされるため、誤ったアンカーは 1 フレーム以内に自己回復する。
...
検出してシンボルを追加し、航法データデコーダがプリアンブル差分でフレーム同期を取り、
定数シンボルオフセットを復元してリードソロモン符号によるエラーチェックを処理する。
フレーム同期後は、復元したシンボルオフセット (`nav->coff`) が信号追尾側へ
フィードバックされ、CSK ピーク探索の縮小に使われる (4.16 節参照)。
...
場合、FFT より安いからである。L6 では、これに加えて CSK コード位相シフトを
探索するための小さな固定サイズのチップ領域 FFT をシンボル毎に実行する。E/P/L 相関は
他の信号と同様に時間領域相関器を使う。
...
- 搬送波ワイプオフ後のサンプルは、標準相関器ではチャンク単位で内部消費する。
  L6 CSK パスと複素コードパスのみ受信機チャネルローカルの作業バッファへ生成する。
...
FFT ヘルパ `sdr_cpx_fft()` は FFT 実装を隠蔽する。信号捕捉は `sdr_corr_fft()` を
通してこれを使い、L6 CSK 信号追尾は固定サイズのチップ領域相関のためにこれを直接使う。
コードベースは、より広い **Pocket SDR** ツールチェーンで
FFTW wisdom の生成と読み込みもサポートしている。受信機経路でアルゴリズム上重要なのは、
コード FFT が一度だけ生成される一方、データ FFT は各ドップラー周波数ビンまたは
各 L6 シンボルごとに生成される点である。
...
L6 も、CSK データがコード位相シフトとして符号化されるため、シンボル毎に広い相関
領域を調べる必要がある。ただしこの探索はチップビニングされたデータ上の小さな固定
サイズ FFT で行い、ループ相関器出力は時間領域相関器から得る。
...
ch_thread()
    -> sdr_ch_update()
        -> search_sig() if SRCH
        -> track_sig()  if LOCK
            -> sdr_corr_std() (搬送波ミックス融合)、または
            -> sdr_mix_carr() -> sdr_corr_std_cpx_code() (E5ABQ)、または
            -> sdr_mix_carr() -> sdr_bin_csk() -> チップ領域FFT
               -> CSKシフト付き sdr_corr_std() (L6D/E)
            -> sync_sec_code()
            -> FLL() or PLL()
            -> DLL()
            -> CN0()
...
| `L6D`, `L6E` | チップ領域 FFT による CSK シンボル検出、CSK シフト付き E/P/L |
...
AltBOC には複素コードバンクを使い、CSK にはチップ領域 FFT のシンボル探索を使う。
